...
import com.strong.appstore.http.ipshift.IpTableCache;
import com.strong.appstore.utils.LogUtils;
import com.strong.appstore.utils.NumberFormatUtil;
...
public class LekeNoHttp extends BaseLekeNoHttp{
    ...
    public LekeNoHttp(int what , Request<String> request , LekeResponse.Listener listener, LekeResponse.ErrorListener errorListener){
        request.addHeader("HOST" , Config.KEY_TUTOR_HOST);
...
//        request.add("ticket", LekeTeacherApplication.getInstance().mParameters.getToken());
    ...
    private String initUrl(String _s , String _m){
        String url = Config.HOME_WORK_NEW;
        /** 获取host值 **/

        String ticket = (String) SPUtils.getInstance().get(Constants.KEY_LOGIN_TICKET , "");
        url = url + "?ticket="
                + ticket + "&_s=" + _s
                + "&_m=" + _m + "&device=android" + "&version="
                + Config.REQUEST_VERSION;

        if (Config.ENVIRONMENT == Config.ApiEnvironment.online) {
    ...
    private String changeHost(String url){
        ...
        LogUtils.d("host is : " + host + " *** ip is : " + requestIp);
